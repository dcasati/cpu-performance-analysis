...
Possible options are:
    try-opening     try to open the xdg-file. On Linux this uses xdg-open. On MacOS: open. 
    copy-perf-data  
"
usage() {
...
main() {
   ...
   if [ -z "${_TARGET_CONTAINER+x}" ]; then
      ...
      CONTAINERS_ARRAY=($(echo $SPEC_CONTAINERS))
      
      if [ "${#CONTAINERS_ARRAY[@]}" -gt 1 ]; then
        _TARGET_CONTAINER=${CONTAINERS_ARRAY[0]}
        echo "Found the following containers on this pod: ${CONTAINERS_ARRAY[@]}"
